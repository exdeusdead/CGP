...
async function getCgpSession() {
  ...
  const response =
    await fetch(
      `${CGP_API}/companion/me`,
      {
        headers: {
          Authorization:
            `Bearer ${saved.cgpToken}`
        }
      }
    );
...
async function loadCgpStatus() {
  ...
  try {
    ...
    status.innerText =
      `Connected: ${session.data.user.id}`;
...
async function saveCgpToken() {

  const token =
    document
      .getElementById("cgpToken")
      .value
      .trim();
